...
class PassCodeWidget extends StatelessWidget {
          ...
          if (state is PinCodeResultState) {
            if (state.isFinish) {
              Future.delayed(const Duration(milliseconds: 500)).then(
                (value) =>
                    passCodeCallback(state.isSuccess, state.failedAttempts),
              );
            }
            if (!state.isSuccess) {
            ...
            Center(
              child: Keyboard(
                ...
                keyboardUIConfig: KeyboardUIConfig(
                    digitFillColor: Get.theme.colorScheme.extraLightShade,
                ...
                rightWidget: Center(
                  child: SvgPicture.asset(IconsSVG.deleteIcon),
                ),
                onLeftWidgetTap: () {
  ...
  bool _isCreating() {
    return status == PassCodeStatus.signInCreate ||
        status == PassCodeStatus.signUpCreate;
  }
