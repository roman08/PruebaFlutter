...
class SessionRepository {
  SessionRepository(
      this._session, this._secureRepository, this._cacheRepository);
  ...
  Future<void> initSessionFromStorage() async {
    final refreshToken = await _secureRepository
        .getFromStorage(StorageConstants.refreshTokenKey);
    final accessToken =
        await _secureRepository.getFromStorage(StorageConstants.accessTokenKey);
    final passCode =
        await _secureRepository.getFromStorage(StorageConstants.passCodeKey);
    final uid = await _secureRepository.getFromStorage(StorageConstants.uidKey);
    final biometricTypeAsString =
        await _secureRepository.getFromStorage(StorageConstants.biometricType);
  ...
  void setBiometricType(BiometricType type) {
    ...
    _secureRepository.addToStorage(
        StorageConstants.biometricType, type.toNameString());
  }
