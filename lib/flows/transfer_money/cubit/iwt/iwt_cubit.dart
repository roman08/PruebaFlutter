...
class IwtCubit extends Cubit<IwtState> {
  ...
  void loadInstructions({int accountId}) async {
    ...
    await for (final resource
        in _iwtRepository.loadInstructions(accountId: accountId)) {
  ...
  void saveFile({int accountId, int iwtId}) async {
    emit(IwtFileSaving());
    await for (final resource
        in _iwtRepository.downloadPdf(accountId: accountId, iwtId: iwtId)) {
      if (resource.status == Status.success) {
        String path;
        ...
        final fileName =
            'IWT ${DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now())}.pdf';
