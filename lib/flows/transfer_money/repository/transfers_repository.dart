...
class TransfersRepository {
  ...
  Future<List<AppContact>> fetchUserContacts() async {
    ...
    final List<String> phoneNumbersToCheck = contacts
        .map((contact) => cleanNumber(contact.phones.first.value))
        .toList();
    ...
    return contacts.map((contact) {
      return AppContact(
          ...
          uid: appNumbers.data
                  .firstWhere(
                      (element) =>
                          cleanNumber(contact.phones.first.value) ==
                          cleanNumber(element.phoneNumber),
                      orElse: () => null)
                  ?.uid ??
              "",
  ...
  Future<List<Contact>> _loadUserAddressBook() async {
    ...
    return contacts
        .where((contact) =>
            contact?.displayName?.isNotEmpty != null &&
            contact.phones.isNotEmpty)
        .toList();
  }

  Future<ContactEntity> findAppContacts(List<String> phones) async {
    final ContactRequestEntity requestModel =
        ContactRequestEntity(phoneNumbers: phones);
